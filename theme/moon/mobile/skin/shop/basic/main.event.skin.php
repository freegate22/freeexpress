...
<div  id="sev">
    ...
    for ($i=0; $row=sql_fetch_array($hresult); $i++)
    {
        ...
        echo '<button type="button" class="sev_text accordion-btn">';
            if ($row['ev_subject_strong']) echo '<strong>';
            echo $row['ev_subject'];
            if ($row['ev_subject_strong']) echo '</strong>';
            echo '<i class="fa fa-chevron-down" aria-hidden="true"></i></button>'.PHP_EOL;
...
<script>
  $(document).ready(function($) {
    $("#idx_ev").find(".accordion-btn").click(function(){
